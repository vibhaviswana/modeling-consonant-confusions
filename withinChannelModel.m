% Within-channel modulation masking model to predict consonant confusions.
...
env_resp = zeros(nconds,ntalkers,nconson,nCFs,Cdur_bins+1);
env_SiSNM_l = zeros(ntalkers,nconson,nCFs,Cdur_bins+1);
env_SiSNM_r = zeros(ntalkers,nconson,nCFs,Cdur_bins+1);
for aTalker = 1:ntalkers
    for aCons = 1:nconson
        ...
        for aCond = 1:nconds
            ...
            for p = 1:nCFs
                CF_hertz = list_CFs(p);
                env_resp(aCond,aTalker,aCons,p,:) = computeENV(resp(p,:),CF_hertz,fs_PSTH);
            end
            if (aCond == 4) % SiDCmod
                ...
                for p = 1:nCFs
                    CF_hertz = list_CFs(p);
                    env_SiSNM_l(aTalker,aCons,p,:) = computeENV(respl(p,:),CF_hertz,fs_PSTH);
                    env_SiSNM_r(aTalker,aCons,p,:) = computeENV(respr(p,:),CF_hertz,fs_PSTH);
                end
            end
        end
    end
...
for aTalker = 1:ntalkers
    for aConsPair = 1:(nconson*nconson)
        ...
        envQuiet = squeeze(env_resp(1,aTalker,aConsHeard,:,:));
        envCond = squeeze(env_resp(1,aTalker,aConsPres,:,:));
        for aCF = 1:nCFs
            ord = 0;
            framelen = 1;
...
save('withinChannelModelOutput','confMat');


%% Helper functions

function outp = computeENV(inp,CF_hertz,fs) % ENV computation

CF_ERB = cams(CF_hertz); % converts Hz to ERB units
BW = invcams(CF_ERB+0.5)-invcams(CF_ERB-0.5);
BW = min(BW,(fs/2)-eps(fs/2));
